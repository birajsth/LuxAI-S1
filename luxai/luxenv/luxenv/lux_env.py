...
from .reward import StatefulMultiReward as RewardSpace
...
class LuxEnvironment(gym.Env):
    ...
    def step(self, action_codes):
        """
        Take this actions, then get the state at the next action
        :param action_code:
        :return:
        """
    ...
    def get_obs_reward_done_info(self) -> Tuple[Game, Tuple[float, float], bool, Dict]:
        ...
        if self.done:
            self.info = {
                ...
                "terminal_state":deepcopy(self.game)
            }
    ...
    def get_reward(self):
        ...
        if self.done:
            if self.game.get_winning_team() == self.learning_agent.team:
                return 1
            else:
                return -1
        return 0
