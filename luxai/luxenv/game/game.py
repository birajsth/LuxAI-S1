...
from .actions import MoveAction, PillageAction, SpawnCartAction, SpawnCityAction, SpawnWorkerAction, \
...
class Game:
    def __init__(self, configs=None, agents=[]):
        ...
        if self.configs['storeReplay']:
            self.start_replay_logging(stateful=True)
        self.reset()
        self.log_file = None
        self.log_dir = "./gamelogs/logs/"
        if not os.path.exists(self.log_dir):
            os.makedirs(self.log_dir)
        

    def start_replay_logging(self, stateful=False, replay_folder="./gamelogs/replays/", replay_filename_prefix="replay"):
        """
    ...
    def validate_command(self, cmd, accumulated_action_stats=None):
        ...
        if isinstance(cmd, SpawnCityAction):
            ...
            if not unit.can_move():
                self.log("Agent tried to move unit on cooldown")
    ...
    def spawn_city_tile(self, team, x, y, city_id=None):
        ...
        if len(adj_same_team_city_tiles) == 0:
            city = City(team, self.configs, self.global_city_id_count + 1, fuel=0)
            ...
            cell.set_city_tile(team, city.id, self.state["turn"])
            city.add_city_tile(cell)
            ...
            city = self.cities[city_id]
            cell.set_city_tile(team, city_id, self.state["turn"])
    ...
    def get_city_tile(self, city_tile_id):
        pos = city_tile_id.split("_")[2:]
        return self.map.get_cell(int(pos[0]), int(pos[1])).city_tile
    ...
    '''
        try:
            ...
            self.log(f"get_city_tile error: {pos[0]}, {pos[1]}")
            self.log(self.map.get_map_string())
            return None
            '''
